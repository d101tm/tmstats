...
if [[ "$dorun" = "yes" ]] ; then
    ...
    if (( $haveclubs == 0 )) ; then
        curl -L -o d42016.csv "https://www.dropbox.com/s/v79wdxyuitxouui/d4tm_clubs_2016-17.csv"
        curl -L -o d4officers.csv "https://docs.google.com/spreadsheets/d/16WdpdhG07rCgRoWr0wKgvSG53k_rjwrMgDx9Rf5BdFk/pub?gid=0&single=true&output=csv"
        ../createalignment.py --outfile d4-2016-full.csv --align d42016.csv
        eval $(../checkfortransition.py newAlignment --tmyear 2016 --newA d4-2016-full.csv)
        eval $(../checkfortransition.py officers --tmyear 2016 --officers file://$PWD/d4officers.csv)
        echo "Building Area/Division page"
        echo ../buildareapage.py $newAlignment $officers
        ../buildareapage.py $newAlignment $officers
        # Creates areasanddivisions.html
        
        echo "Building Clubs by City page"
        ../listclubsbycity.py $newAlignment
        
        echo "Running Club Change Report"
        ../runclubchanges.sh Sat

        echo "Making map"
        echo ../makemap.py -q $newAlignment
        ../makemap.py -q $newAlignment
