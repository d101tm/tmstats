...
if __name__ == "__main__":
    import tmparms, latest, sys, csv
    from datetime import datetime
    from tmutil import cleandate, getClubBlock
    import tmglobals
    import os
    ...
    parms.add_argument('--pct', dest='pct', nargs='+', type=float,
                       help='Threshold to qualify (in percent) for each level.', default='75.0')
    ...
    parms.add_argument('--name', dest='name', nargs='+', type=str,
    ...
    if len(parms.pct) != len(parms.earns):
        sys.stderr.write(
            'The number of values for PCT and EARNS must be the same, but we got %d in PCT and %d in EARNS\n' % (
    ...
    if parms.name:
        if len(parms.pct) != len(parms.name):
            sys.stderr.write(
                'The number of values for PCT and NAME must be the same, but we got %d in PCT and %d in NAME\n' % (
    ...
    query = "SELECT c.clubnumber, c.clubname,  %s, c.membase, c.division, c.area   from clubperf c  inner join distperf d on c.clubnumber = d.clubnumber and c.asof = '%s' and d.asof = '%s' order by c.division, c.area, c.clubnumber" % (rtype, asof, asof)
    curs.execute(query)
    ...
    # Write the raw data
    outfile = open(os.path.join(parms.workdir, parms.outfileprefix + '.csv'), 'w')
    ...
    for each in levels:
        if len(each.winners) > 0:
            ...
            res = '<p><b>Congratulations</b> to\n' + winners + '\n for renewing' + (
                ' at least' if each.pct < 100.0 else '') + ' %d%% ' % each.pct + 'of their base membership and earning ' + (
                          parms.format % each.earns)
            if each.name:
