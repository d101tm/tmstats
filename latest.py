...
statement = 'select t.month, l.latest FROM %s t INNER JOIN (select max(loadedfor) as latest FROM loaded WHERE tablename="%s") l ON t.asof = l.latest GROUP BY t.month, l.latest' % (parms.table, parms.table)
